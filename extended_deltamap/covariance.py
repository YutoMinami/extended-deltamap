...
class Covariance:
    ...
    def ReturnBetalm(self, ell, m, theta, plus=True):
        ...
        betalm = (
            2.0
            * numpy.sqrt((2.0 * ell + 1) / (2.0 * ell - 1) * (pow(ell, 2) - pow(m, 2)))
            * (sgn * m / pow(numpy.sin(theta), 2) + 1.0 / numpy.tan(theta) / numpy.sin(theta))
        )
        return betalm
    ...
    def ReturnF1lm(self, ell, m, theta):
        first = (
            ...
            * lpmv(int(m), int(ell), numpy.cos(theta))
        )
        # print(lpmv( int(m), int(ell), numpy.cos(theta)))
        second = (
            (ell + m) * numpy.cos(theta) / pow(numpy.sin(theta), 2) * lpmv(int(m), int(ell - 1), numpy.cos(theta))
            if abs(m) <= (ell - 1)
            else 0.0
        )
    ...
    def Return2Ylm(self, ell, m, theta, phi, plus=True):
        ...
        aterm = alphalm * sph_harm_func(m, ell, phi, theta)
        bterm = 0.0 if (ell - 1) < abs(m) else betalm * sph_harm_func(m, ell - 1, phi, theta)
        if self.verbose >= 2:
            print(
                "l m",
                ell,
                m,
                "Yl,m Yl-1,m",
                sph_harm_func(m, ell, phi, theta),
                sph_harm_func(m, ell - 1, phi, theta),
                "alphalm : ",
                alphalm,
                "betalm : ",
                betalm,
    ...
    def ReturnEllSum(self, YY):
        ...
        for idx, ell in enumerate(range(self.lmin, self.lmax + 1, 1)):
            YYl[:, :, idx] = numpy.sum(
                YY[:, :, (ell - self.lmin) * (ell + self.lmin) : (ell - self.lmin) * (ell + self.lmin) + 2 * ell + 1], axis=2
            )
        return YYl
    ...
    def ReturnWWlArray(self, i, j):
        ...
        for idx, ell in enumerate(range(self.lmin, self.lmax + 1, 1)):
            WWl[idx] = sum(
                WWarray[(ell - self.lmin) * (ell + self.lmin) : (ell - self.lmin) * (ell + self.lmin) + 2 * ell + 1]
            )
        return WWl
    ...
    def ReturnXXlArray(self, i, j):
        ...
        for idx, ell in enumerate(range(self.lmin, self.lmax + 1, 1)):
            XXl[idx] = sum(
                XXarray[(ell - self.lmin) * (ell + self.lmin) : (ell - self.lmin) * (ell + self.lmin) + 2 * ell + 1]
            )
        return XXl
    ...
    def ReturnWXlArray(self, i, j):
        ...
        for idx, ell in enumerate(range(self.lmin, self.lmax + 1, 1)):
            WXl[idx] = sum(
                WXarray[(ell - self.lmin) * (ell + self.lmin) : (ell - self.lmin) * (ell + self.lmin) + 2 * ell + 1]
            )
        return WXl
    ...
    def ReturnXWlArray(self, i, j):
        ...
        for idx, ell in enumerate(range(self.lmin, self.lmax + 1, 1)):
            XWl[idx] = sum(
                XWarray[(ell - self.lmin) * (ell + self.lmin) : (ell - self.lmin) * (ell + self.lmin) + 2 * ell + 1]
            )
        return XWl
    ...
    def CalcCovArray(self, Cl):
        ...
        self.QU = -1.0 * numpy.sum(WXl[..., :] * Cl[1][: WXl.shape[2]] * pow(pw_l, 2), axis=2)  ##EE
        self.QU += numpy.sum(XWl[..., :] * Cl[2][: XWl.shape[2]] * pow(pw_l, 2), axis=2)  ##BB
        self.QU = -1.0 * numpy.sum(XWl[..., :] * Cl[1][: XWl.shape[2]] * pow(pw_l, 2), axis=2)  ##EE
    ...
    def CalcCov(self, Cl):
        ...
        for i in range(self.size):
            for j in range(self.size):
                ...
                self.QU[i, j] = -1.0 * sum(WXl * Cl[1][: len(WXl)])  ##EE
                self.QU[i, j] += sum(XWl * Cl[2][: len(XWl)])  ##BB
                self.QU[j, i] = -1.0 * sum(XWl * Cl[1][: len(XWl)])  ##EE
    ...
    def CalcCov02(self, Cl):
        ...
        for i in range(self.size):
            ...
            self.QU[i, :] = -1.0 * numpy.sum(WXl[..., :] * Cl[1][: WXl.shape[2]] * pow(pw_l, 2), axis=2)[0]  ##EE
